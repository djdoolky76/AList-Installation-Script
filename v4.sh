...
INSTALL() {
  ...
  curl -L ${GH_PROXY}https://github.com/djdoolky76/alist/releases/download/seedbx-version/alist-linux-musl-$ARCH.tar.gz -o /tmp/alist.tar.gz $CURL_BAR
...
#   echo -e "Initial management password：${GREEN_COLOR}$(echo $get_password | awk -F'your password: ' '{print $2}')${RES}"
  ...
  echo -e "Stop the service：${GREEN_COLOR}systemctl stop alist${RES}"
